...
class DataExplorer:
    """
    Class for exploring output of export.py script. It provides methods for accessing the cells by id and time frame.
    You can simply access specific descriptor for a cell, or get the whole timeline of a cell. 
    Example usage:
    ```
    ...
    def GetPathToFrame(self, frame : int):
        """Returns the path to the frame directory. If the frame is out of bounds, returns None"""
        if (frame < 0 or frame >= len(self.frames)):
            return None
        return f"{self.frames[frame]}"
    

    def GetCellAtFrame(self, frame : int, cell_id : int)->(np.ndarray, np.ndarray):
        """Returns the image and mask of a cell at a given frame. Returns None, None if the cell does not exist"""
    ...
    def GetDescriptorsAtFrame(self, frame : int) -> dict:
        """Returns the descriptors of all cells at a given frame. Returns None if the frame does not exist"""
        frame_path = self.GetPathToFrame(frame)
    ...
    def GetDescriptorsForCell(self, frame : int, cell_id : int) -> dict:
        """Returns all descriptors of a cell at a given frame. Returns None if the cell or frame does not exist"""
    ...
    def GetCellDescriptor(self, frame : int, cell_id : int, descriptor_name : str, dict_val : str =""):
        """
            Returns specific descriptor for given cell in frame. 
            - dict_val - if set and the descriptor is a dictionary, returns the value of the key dict_val
    ...
    def GetAllCellsInFrame(self, frame : int):
        """Returns all celll ids in a given frame. Returns None if the frame does not exist"""
        descriptors = self.GetDescriptorsAtFrame(frame)
        if descriptors is None:
            return None
    ...
    def GetCellTimeline(self, cell_id : int):
        """
        Returns 2 lists:
            - image timeline - list of images of the cell in each frame, in order
    ...
    def GetCellDescriptorTimeline(self, cell_id, descriptor_name, dict_val=""):
        ...
        timeline = []
        for frame in range(len(self.frames)):
            descriptor = self.GetCellDescriptor(frame, cell_id, descriptor_name, dict_val)
